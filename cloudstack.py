...
# (c) 2017, Marc Guillen <wacko@wacko.ch>
#
...
class CloudStackInventory(object):
    ...
    def get_host(self, name, project_id=None, domain_id=None):
        ...
        for host in hosts['virtualmachine']:
            ...
            if name == host_name:
                data['display_name'] = host['displayname']

                # Default ansible variables
                data['ansible_port'] = 22
                data['ansible_connection'] = 'paramiko'

                # API retrieved varaibles
                ...
                for nic in host['nic']:
                    ...
                    if nic['isdefault']:
                        ...
                        data['ansible_host'] = nic['ipaddress']

                # Set the user and python path
                if 'CoreOS' in host['templatename']:
                    data['ansible_user'] = 'core'
                    data['ansible_python_interpreter'] = '/opt/bin/python'
                elif 'OpenBSD' in host['templatename']:
                    data['ansible_python_interpreter'] = '/usr/local/bin/python'
                else:
                    data['ansible_user'] = 'root'
        ...
        for host in hosts['virtualmachine']:
            ...
            for tag in host['tags']:
                if tag['key'] in self.GROUP_TAGS:
                    ...
                    data[tag['value']]['hosts'].append(host_name)

            # Default ansible variables
            data['_meta']['hostvars'][host_name]['ansible_port'] = 22
            data['_meta']['hostvars'][host_name]['ansible_connection'] = 'paramiko'

            # API retrieved variables
            if 'group' in host:
                data['_meta']['hostvars'][host_name]['group'] = host['group']
            data['_meta']['hostvars'][host_name]['display_name'] = host['displayname']
            ...
            data['_meta']['hostvars'][host_name]['tags'] = {}
            for tag in host['tags']:
                if tag['key'] in self.GROUP_TAGS:
                    data['_meta']['hostvars'][host_name]['tags'][tag['key']] = tag['value']
            ...
            for nic in host['nic']:
                ...
                if nic['isdefault']:
                    ...
                    data['_meta']['hostvars'][host_name]['ansible_host'] = nic['ipaddress']

            # Set the user and python path
            if 'CoreOS' in host['templatename']:
                data['_meta']['hostvars'][host_name]['ansible_user'] = 'core'
                data['_meta']['hostvars'][host_name]['ansible_python_interpreter'] = '/opt/bin/python'
            elif 'OpenBSD' in host['templatename']:
                data['_meta']['hostvars'][host_name]['ansible_python_interpreter'] = '/usr/local/bin/python'
            else:
                data['_meta']['hostvars'][host_name]['ansible_user'] = 'root'
                data['_meta']['hostvars'][host_name]['ansible_python_interpreter'] = '/usr/bin/python'
